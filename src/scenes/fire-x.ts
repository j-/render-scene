...
export default class extends Scene {
  ...
  protected readonly MAX_TEMPERATURE = this.CHANNELS.length / 3 - 1;
  ...
  doSource (frame: Frame) {
    const { buffer, BUFFER_LENGTH, BUFFER_WIDTH, MAX_TEMPERATURE } = this;
  ...
  drawBuffer () {
    ...
    for (let i = 0; i < BUFFER_LENGTH; i++) {
      ...
      if (white) {
        ...
        bufferID.data[i * 4 + 0] = this.getChannel(0, temperature);
        bufferID.data[i * 4 + 1] = this.getChannel(1, temperature);
        bufferID.data[i * 4 + 2] = this.getChannel(2, temperature);
      }
    }
  ...
  getChannel (channel: number, temperature: number) {
    return this.CHANNELS[channel + temperature * 3];
  }
}
